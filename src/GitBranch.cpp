...
constexpr char               EnvVarStarship[] = "STARSHIP_CONFIG";
...
void WINAPI SetStartupInfoW(const PluginStartupInfo *psi)
{
    ...
    SetEnvironmentVariableA(EnvVarStarship, sh_config.string().c_str());
...
intptr_t WINAPI ProcessSynchroEventW(const struct ProcessSynchroEventInfo*)
{
    ...
    if (PreviousDir != directory || Timeout()) {
        ...
        if (!directory.empty()) {
            ...
            auto cmdres = raymii::Command::exec("starship prompt -p " + git_dir.string());

            branch = cmdres.output;
            if (!branch.empty()) {
                branch = " (" + branch + ")";
            }
        }
